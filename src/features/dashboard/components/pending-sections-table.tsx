...
function SectionTableContent({ rows }: { rows: SectionPerformanceRow[] }) {
  if (!rows.length) {
    return (
...
export function PendingSectionsTable({
  rows,
  embedded = false,
}: PendingSectionsTableProps) {
  if (embedded) {
    return (
      <div>
        <h3 className="mb-2 text-sm font-semibold text-foreground">
          Top Pending Internal Sections
        </h3>
        <SectionTableContent rows={rows} />
      </div>
    );
  }
