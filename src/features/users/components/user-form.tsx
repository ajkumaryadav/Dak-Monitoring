...
export function UserForm({ mode, options, user }: UserFormProps) {
  ...
  const roleOptions = useMemo(
    () =>
      options.roles.map((r) => ({
        value: mapRoleSlugToAppRole(r.slug),
        label: getRoleLabel(mapRoleSlugToAppRole(r.slug)),
      })),
    [options.roles]
  );
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <div className="grid gap-2">
          <Label htmlFor="departmentId">
            Department {isDepartmentUser ? "(required)" : ""}
          </Label>
          <select
            id="departmentId"
            name={departmentDisabled ? undefined : "departmentId"}
            value={departmentId}
            disabled={departmentDisabled}
            required={isDepartmentUser}
            onChange={(e) => setDepartmentId(e.target.value)}
            className={inputClassName}
          >
            <option value="">
              {departmentDisabled ? "Not applicable" : "Select department"}
            </option>
        ...
        <div className="grid gap-2">
          ...
          >
            <option value="">
              {sectionDisabled ? "Not applicable" : "Select section"}
            </option>
