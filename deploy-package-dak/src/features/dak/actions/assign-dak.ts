...
export async function assignDak(
  ...
  try {
    ...
    const recalledSet = await getRecalledDakIdSet([parsed.data.dakId]);
    if (recalledSet.has(parsed.data.dakId)) {
      return {
        success: false,
        message: "Cannot assign this DAK because it has been recalled by the registry operator.",
      };
    }
