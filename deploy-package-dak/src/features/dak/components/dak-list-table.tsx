    ...
    <Table>
      <TableHeader>
        <TableRow className="border-primary/10 bg-primary/[0.04] hover:bg-primary/[0.04]">
          ...
          {showRecallAction && <TableHead className="px-4 text-right">Action</TableHead>}
        </TableRow>
      </TableHeader>
      <TableBody>
        {entries.map((entry) => (
            ...
            {showRecallAction && (
              <TableCell className="px-4 text-right">
                {(() => {
                  ...
                  if (["received", "assigned", "under_process", "in_progress"].includes(entry.status)) {
                    return (
                      <DakRecallButton
                        dakId={entry.id}
                        dakNumber={entry.dak_number}
